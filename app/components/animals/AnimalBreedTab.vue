<template>
  <div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-12">
      ...
      <span class="ml-3 text-gray-600">Loading animal breeds...</span>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
      <p class="font-medium">Failed to load animal breeds</p>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-if="breeds.length === 0">
              <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                No animal breeds found. Add your first breed to get started.
              </td>
            </tr>
          </tbody>
        </table>
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditing.value && editingUuid.value) {
      if (isOnline.value) {
        const response = await $apiFetch<{ status: string; message: string; data: AnimalBreed }>(`/api/v1/settings/animals/breeds/${editingUuid.value}`, {
          method: 'PUT',
          body: payload
        })
      ...
      if (isOnline.value) {
        const response = await $apiFetch<{ status: string; message: string; data: AnimalBreed }>('/api/v1/settings/animals/breeds', {
          method: 'POST',
          body: payload
        })
...
const fetchAnimalTypes = async () => {
  try {
    if (isOnline.value) {
      await $apiFetch('/sanctum/csrf-cookie')
      const response = await $apiFetch<{ data: AnimalType[] }>('/api/v1/settings/animals/types/list')
...
const fetchData = async () => {
  ...
  try {
    if (isOnline.value) {
      await $apiFetch('/sanctum/csrf-cookie')
      const response = await $apiFetch<{ data: AnimalBreed[] }>('/api/v1/settings/animals/breeds/list')
      breeds.value = response.data || response as unknown as AnimalBreed[]
    } else {
      breeds.value = []
    }
  } catch (err: any) {
    console.error('Failed to fetch animal breeds:', err)
...
const deleteItem = async (uuid: string) => {
  ...
  try {
    if (isOnline.value) {
      await $apiFetch('/sanctum/csrf-cookie')
      await $apiFetch(`/api/v1/settings/animals/breeds/${uuid}`, { method: 'DELETE' })
    }
