...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditing.value && editingUuid.value) {
      if (isOnline.value) {
        const response = await $apiFetch<{ status: string; message: string; data: AnimalType }>(`/api/v1/settings/animals/types/${editingUuid.value}`, {
          method: 'PUT',
          body: form.value
        })
        const index = animalTypes.value.findIndex(a => a.uuid === editingUuid.value)
        if (index !== -1) {
      ...
      if (isOnline.value) {
        const response = await $apiFetch<{ status: string; message: string; data: AnimalType }>('/api/v1/settings/animals/types', {
          method: 'POST',
          body: form.value
        })
...
const deleteItem = async (uuid: string) => {
  if (!confirm('Are you sure you want to delete this animal type?')) return
  
  try {
    if (isOnline.value) {
      await $apiFetch('/sanctum/csrf-cookie')
      await $apiFetch(`/api/v1/settings/animals/types/${uuid}`, { method: 'DELETE' })
    }
